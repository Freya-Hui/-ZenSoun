...
import { MessageSquare, Heart, Bookmark, Download, Sparkles, Send, ShieldAlert, BadgeCheck, Music, Play, Square, X } from 'lucide-react';
...
export default function CommunityCenter({
  ...
  // Stop playing live previews when tab changes / unmounts
  useEffect(() => {
    return () => {
      audioEngine.stopAll();
    };
  ...
  const handleTogglePreview = (recipe: SoundRecipe) => {
    audioEngine.ensureContext();
    if (playingPreviewId === recipe.id) {
      audioEngine.stopAll();
      setPlayingPreviewId(null);
    } else {
      // Pause global track player first so they do not overlap
      window.dispatchEvent(new CustomEvent('zensound-pause'));

      // Clear any other ambient sounds in synthesizer
      audioEngine.stopAll();

      // Setup recipe parameters
      recipe.noises.forEach(n => {
        if (n.isActive) {
          audioEngine.setNoiseVolume(n.id, true, n.volume);
        } else {
          audioEngine.setNoiseVolume(n.id, false, 0);
        }
      });

      // Update binaural program
      audioEngine.updateBrainwave(recipe.purpose, 0.4);

      setPlayingPreviewId(recipe.id);
    }
  };
  ...
  return (
    <div className={`flex flex-col h-full transition-colors duration-300 ${isDark ? 'bg-[#0a0f1d] text-gray-200' : 'bg-[#faf9f6] text-stone-800'}`}>
      ...
      <div className={`flex items-center justify-between px-4 py-2 border-b font-sans shadow-xs ${isDark ? 'bg-[#0d1629] border-slate-900/60' : 'bg-[#fdfbf6] border-[#dacdb9]/60'}`} id="community_filters">
        <div className={`flex rounded-lg p-0.5 border ${isDark ? 'bg-slate-950 border-slate-900' : 'bg-[#f2e7d0]/40 border-[#dacdb9]/45'}`}>
          <button
            ...
            className={`px-3 py-1 text-xs font-md rounded-md transition-all cursor-pointer ${
              activeRecipeFilter === 'all'
                ...
                : isDark ? 'text-gray-455 hover:text-gray-300' : 'text-[#826e5e] hover:text-[#4e3629]'
            }`}
          >
            全部交流
          </button>
        ...
        <span className={`text-[10px] font-mono ${isDark ? 'text-gray-500' : 'text-[#826e5e]'}`}>
          共 {posts.length} 条讨论
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 pb-20" id="posts_scroller">
        {/* Post Input Box */}
        <div className={`rounded-xl border p-4 shadow-sm flex flex-col gap-3 ${isDark ? 'bg-[#0f172a] border-slate-800/80 shadow-lg' : 'bg-[#fdfbf6] border-[#dacdb9]/80 shadow-sm'}`}>
          ...
          <div className="relative">
            <textarea
              ...
              rows={3}
              className={`w-full rounded-xl px-3 py-2.5 text-xs font-sans focus:outline-none border ${isDark ? 'bg-[#070b13] border-slate-900 text-white placeholder-gray-600 focus:border-amber-500/50' : 'bg-[#f5ead5] border-[#dacdb9] text-[#4e3629] placeholder-stone-400 focus:border-[#a67c52]'}`}
            />
          </div>
        ...
        <div className="space-y-4" id="community_posts_feed">
          {filteredPosts.map(post => {
            return (
              <div 
                key={post.id}
                className={`rounded-xl p-4 transition-all border ${isDark ? 'bg-[#0f172a]/60 border-slate-900 hover:border-slate-800' : 'bg-[#fdfbf6]/65 border-[#dacdb9]/80 hover:border-[#a67c52]/30 shadow-xs'}`}
              >
                {/* Post Author area */}
                <div className="flex justify-between items-start mb-2.5">
                  <div className="flex items-center gap-2.5">
                    <div className={`w-8 h-8 rounded-full overflow-hidden flex items-center justify-center text-xs border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-[#f5ead5] border-[#dacdb9] text-stone-700'}`}>
                      {post.userAvatar}
                    </div>
                    <div>
                ...
                {post.recipe && (
                  <div className={`p-3 rounded-xl border border-l-[3px] border-[#a67c52] flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between mb-3.5 ${isDark ? 'bg-[#070b13] border-slate-900' : 'bg-[#faf6ed] border-[#ecdcb9]/55'}`}>
                    <div className="flex-1 min-w-0 pr-2">
                       <div className="flex items-center gap-1.5 mb-1">
                        <Music className={`w-3.5 h-3.5 ${isDark ? 'text-amber-400' : 'text-[#a67c52]'}`} />
                        <span className={`text-xs font-black font-sans truncate ${isDark ? 'text-gray-200' : 'text-stone-850'}`}>
                          {post.recipe.name}
                        </span>
                      </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto mt-2 sm:mt-0">
                      {/* Audition Button */}
                      <button
                        onClick={() => handleTogglePreview(post.recipe!)}
                        className={`px-2.5 py-1.5 rounded-lg text-[10px] font-sans font-bold border flex items-center gap-1 cursor-pointer transition-all ${
                          playingPreviewId === post.recipe.id
                            ? 'bg-amber-500 text-slate-950 border-amber-500 animate-pulse font-extrabold shadow-md'
                            : isDark
                              ? 'bg-slate-900/80 hover:bg-slate-800 text-amber-500 border-slate-800'
                              : 'bg-white hover:bg-[#faf6ed] text-[#a67c52] border-[#dacdb9]/55'
                        }`}
                        title="试听配方心流声波"
                      >
                        {playingPreviewId === post.recipe.id ? (
                          <>
                            <Square className="w-2.5 h-2.5 fill-current" /> 停止试听
                          </>
                        ) : (
                          <>
                            <Play className="w-2.5 h-2.5 fill-current" /> 试听配方
                          </>
                        )}
                      </button>
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1.5 rounded-lg text-[10px] font-sans font-bold border flex items-center gap-1.5 cursor-pointer transition-all ${isDark ? 'bg-gradient-to-r from-slate-950 to-slate-900 hover:bg-slate-800 text-amber-500 border-slate-800' : 'bg-[#a67c52]/15 hover:bg-[#a67c52]/25 text-[#a67c52] border-[#a67c52]/30 font-bold'}`}
                      >
                        <Download className="w-2.5 h-2.5" /> 一键装配
                      </button>
                    </div>
                  </div>
                )}
                ...
                <div className={`flex justify-between items-center text-xs border-t pt-3 ${isDark ? 'text-gray-500 border-slate-900/60' : 'text-stone-500 border-[#dacdb9]/30'}`}>
                ...
                {post.comments.length > 0 && (
                  <div className={`rounded-lg p-2.5 mt-3 space-y-1.5 border ${isDark ? 'bg-[#070b13]/40 border-slate-950' : 'bg-[#f4ebd9]/80 border-[#ecdcb9]/40'}`}>
                    {post.comments.map(c => (
